...
//                       <UserGroupIcon
//                         className="h-5 w-5 sm:h-6 sm:w-6 text-purple-900"
//                         aria-hidden="true"
//                       />
//                     </div>
...
const FollowModal = ({ open, setOpen, username, statsToShow }) => {
  ...
  const { data, isLoading, error } = useQuery(
    ...
    async () => {
      const { data } = await axios.get(
        `${baseURL}/api/profile/${username}/${statsToShow.toLowerCase()}`
      );
      return data;
    },
    {
              ...
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <div className="flex flex-col mt-4">
                  {isLoading ? (
                    ...
                    <p className="text-red-500">Error loading {statsToShow}</p>
                  ) : data?.length === 0 ? (
                    <p>No {statsToShow} found</p>
                  ) : (
                    <div className="space-y-3">
                      {data?.map((item) => (
                        <Link
